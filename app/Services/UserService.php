...
class UserService
{
    ...
    public function create(UserCreateRequest $request)
    {
        ...
        try {
            ...
            if (!$user) {
                $user = $this->repository->create($request->except(['role_id', 'image', 'images', 'docs', 'pkcs7']));
                $userHistoryId = $this->createUserActionHistory(user_id: $user->id, comment: $comment, date: now(), status: 1, type: UserActionHistory::TYPE_CREATE, additional_info: $eimzoSign['pkcs7b64']);
                ...
                if ($request->images) {
                    ...
                    $userHistory->images()->createMany(array_map(fn($path) => ['url' => $path], $paths));
                }
                if ($request->docs) {
...
//            CreateNotifiacation::dispatch($data, $this->userId)->afterCommit();
    ...
    private function createUserActionHistory(int $user_id, ?string $comment, ?string $date, int $status, int $type, mixed $additional_info = null)
    {
        return $this->historyService->createHistory(guid: $user_id, status: $status, type: $type, date: $date, comment: $comment, additionalInfo: $additional_info);
    }
